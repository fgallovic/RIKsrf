...
! (i.e. model by Ruiz et al., 2011, with some modifications)
...
!SUBSOURCE PARAMETERS:
    ...
    REAL,ALLOCATABLE,DIMENSION(:):: SUBposL,SUBposW,SUBsize,SUBslip,SUBmoment,SRstressdrop,SUBrisetime,SUBmvr,SUBruptime,SUBnuclL,SUBnuclW
    INTEGER,ALLOCATABLE,DIMENSION(:):: SUBno
    INTEGER SUBtot,idum
!SOURCE PARAMETERS:
    REAL LF,WF,L,W,smL,smW,LWratio,M0,L0,vrsubfact,aparam,dt
    REAL,ALLOCATABLE,DIMENSION(:):: SRl,SRw,SRelem,SRmu,SRslip,SRmoment,SR,STF
    INTEGER NSR,NT
!others
    REAL ran2,dum,dumL,dumW,dumphi,dumr,totmoment,time,compRT,meanVR,ruptime,ruptimeSR,momentcorr
...
!          endif
          ...
          if(SUBmin==1.and.i==1)then   ! If SUBMIN==1, then the largest subsources must have centers in the middle of the fault
            SUBposW(k)=W/2.
        ...
        do m=1,NSR
          ...
          if(dum>0.)then
            ...
            SRstressdrop(m)=SRstressdrop(m)-SRmu(m)/24.*7.*PI/1000.
          endif
        enddo
      enddo
    enddo
    ...
    do i=1,NSR
      write(201,'(10E13.5)')SRl(i),SRw(i),SRslip(i),SRmoment(i),compRT(SRl(i),SRw(i))
    enddo
    close(201)
...
!        enddo
        ...
        SUBruptime(k)=compRT(SUBnuclL(k),SUBnuclW(k))
    ...
    do k=1,SUBtot
      write(201,'(10E13.5)')SUBposL(k),SUBposW(k),SUBsize(k),SUBmoment(k),compRT(SUBposL(k),SUBposW(k)),SUBrisetime(SUBtot),SUBmvr(SUBtot)
    enddo
    close(201)
...
!normalize and calculate cumulative distribution
    ...
    FUNCTION compRT(x,y)     ! calculate rupture time in a layered model
    USE ruptveloc
    IMPLICIT NONE
    REAL,PARAMETER:: pi2=3.14159/2.
    REAL x,y,compRT
    REAL rtbis,func,alpha
    INTEGER iy,i
    EXTERNAL func
! locate stf point in the vr layers
    iy=1
    do i=1,nlayers
      if(hvr(i)<y) iy=i+1
    enddo
    if(iy==ihw)then    ! stf point in the layer with the hypocenter
      compRT=sqrt((x-hl)**2+(y-hw)**2)/(vr(iy))
    else               ! stf point 
      alpha=rtbis(func,-pi2,pi2,.0001,y,iy,x-hl)
      if(iy<ihw)then    !alpha is considered at y
        compRT=(hvr(iy)-y)/cos(alpha)+(hw-hvr(ihw-1))/sqrt(1.-(sin(alpha)*vr(ihw)/vr(iy))**2)/(vr(ihw)/vr(iy))
        do i=iy+1,ihw-1
          compRT=compRT+(hvr(i)-hvr(i-1))/sqrt(1.-(sin(alpha)*vr(i)/vr(iy))**2)/(vr(i)/vr(iy))
        enddo
        compRT=compRT/vr(iy)
      else              !alpha is considered at hw
        compRT=(hvr(ihw)-hw)/cos(alpha)+(y-hvr(iy-1))/sqrt(1.-(sin(alpha)*vr(iy)/vr(ihw))**2)/(vr(iy)/vr(ihw))
        do i=ihw+1,iy-1
          compRT=compRT+(hvr(i)-hvr(i-1))/sqrt(1.-(sin(alpha)*vr(i)/vr(ihw))**2)/(vr(i)/vr(ihw))
        enddo
        compRT=compRT/vr(ihw)
      endif     
      continue
    endif
    END
    
    FUNCTION func(alpha,y,iy,xshoot)
    USE ruptveloc
    IMPLICIT NONE
    REAL alpha,y,y1,h1,func,xshoot
    INTEGER iy,i
    if(iy<ihw)then    !alpha is considered at y
      y1=(hvr(iy)-y)*tan(alpha)
      h1=(hw-hvr(ihw-1))*sin(alpha)*vr(ihw)/vr(iy)/sqrt(1.-(sin(alpha)*vr(ihw)/vr(iy))**2)
      func=(y1+h1)
      do i=iy+1,ihw-1
        func=func+(hvr(i)-hvr(i-1))*sin(alpha)*vr(i)/vr(iy)/sqrt(1.-(sin(alpha)*vr(i)/vr(iy))**2)
      enddo
    else              !alpha is considered at hw
      y1=(y-hvr(iy-1))*sin(alpha)*vr(iy)/vr(ihw)/sqrt(1.-(sin(alpha)*vr(iy)/vr(ihw))**2)
      h1=(hvr(ihw)-hw)*tan(alpha)
      func=(y1+h1)
      do i=ihw+1,iy-1
        func=func+(hvr(i)-hvr(i-1))*sin(alpha)*vr(i)/vr(ihw)/sqrt(1.-(sin(alpha)*vr(i)/vr(ihw))**2)
      enddo
    endif
    func=func-xshoot
    END
...
! Numerical recipes    
      FUNCTION rtbis(func,x1,x2,xacc,y,iy,xshoot)
      INTEGER JMAX
      ...
      REAL y,xshoot
      INTEGER iy
      fmid=func(x2,y,iy,xshoot)
      f=func(x1,y,iy,xshoot)
